...
open class CometChatGroups: CometChatListBase {
    ...
    public override func viewDidLoad() {
        ...
        setupTableView(style: .grouped, withRefreshControl: true)
    ...
    func handleSelection(for group: Group, at indexPath: IndexPath) {
        if selectionMode == .none {
            ...
            if !group.hasJoined, group.groupType == .public {
                ...
                viewModel.joinGroup(withGuid: group.guid, name: group.name ?? "", groupType: group.groupType, password: "", indexPath: indexPath, completion: { [weak self] in
                    guard let this = self else { return }
                    DispatchQueue.main.async {
                        group.hasJoined = true
                        this.hideJoiningGroupAlert(completion: {
                            this.onItemClick?(group, indexPath)
                        })
                    }
                })
